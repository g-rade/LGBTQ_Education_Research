...
primary_total2 <- full_join(primary_total, primary_total1) %>%
  filter(row_number() != 4)

## plot it with obergefell on the x, not really sure what these plots mean
ggplot(primary_total2, aes(x=lawrence, y=pred_prob))+
geom_ribbon(aes(ymin = pred_lower, ymax = pred_upper),fill = "grey70")+
  geom_point() +
...
ggplot(secondary_negative, aes(x=evangelical_pop, y=pred_prob, color=obergefell)) +
  geom_smooth(method="lm") +
  labs(title = "Predicted Probability of Adoption by Evangelical Population and Obergefell", x = "Evangelical Population", y = "Predicted Probability of Adoption", color = "Obergefell v. Hodges")
