...
state_data <- import("./covariates.csv") ## risk set we used for modeling
```
...
population_total <- state_data %>% 
  select(year, state_name, population_total) %>% ## select variables of interest
  group_by(state_name) %>% ## group_by state
...
pollib_median <- state_data %>% 
  select(year, state_name, pollib_median) %>% 
  group_by(state_name) %>% 
  filter(!is.na(pollib_median)) %>% 
  summarize(min_year = min(year), max_year = max(year)) ## need to get 1987 for all, and beyond 2014

incomepcap <- state_data %>% 
  select(year, state_name, incomepcap) %>% 
  group_by(state_name) %>% 
...
std_bowen1 <- state_data %>% 
  select(year, state_name, std_bowen1) %>% 
  group_by(state_name) %>% 
...
std_bowen2 <- state_data %>% 
  select(year, state_name, std_bowen2) %>% 
  group_by(state_name) %>% 
  filter(!is.na(std_bowen2)) %>% 
  summarize(min_year = min(year), max_year = max(year)) ## need to get 1987 for all, and beyond 2016

unified <- state_data %>% 
  select(year, state_name, unified) %>% 
  group_by(state_name) %>% 
...
std_init_use <- state_data %>% 
  select(year, state_name, std_init_use) %>% 
  group_by(state_name) %>% 
...
evangelical_pop <- state_data %>% 
  select(year, state_name, evangelical_pop) %>% 
  group_by(state_name) %>% 
...
#   select(year, state_name, nbrs_lag) %>% 
#   group_by(state_name) %>% 
#   filter(is.na(nbrs_lag))
# 
# nbrs2 <- state_data %>% 
#   select(year, state_name, nbrs) %>% 
#   group_by(state_name) %>% 
#   filter(is.na(nbrs))
...
- need to add the exisiting data for black/white/hispanic/male/female populations

- std_init_use missing for many states from 2016-2023, need to look into that becuase I have the data through 2022
