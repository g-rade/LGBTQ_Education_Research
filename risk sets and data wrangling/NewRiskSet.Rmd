...
population <- read_csv("datasets/population_only2022.csv") ## population
...
covariates <- covariates %>% 
  full_join(pollib, by = c("state_name", "state", "year")) ## ## join with income 1850 observations (correct) (left join to drop the fips var)

covariates <- covariates %>% 
  full_join(bowen, by = c("state_name", "state", "year")) ## join with bowen standards

covariates <- covariates %>% 
  full_join(unified, by = c("state_name", "state", "year")) ## join with unified, 1850 observations

covariates <- covariates %>% 
  full_join(initiatives, by = c("state_name", "state", "year")) %>% 
  select(-c(mean, sd)) ## join with initiatives, 1850 observations (stay winning)

covariates <- covariates %>% 
  full_join(evangelical2) ## join with evangelical, 1850 observations (slaying)

covariates <- covariates %>% 
  full_join(crosswalk) ## join the crosswalk dataset to fill in the missing values of the fips codes and stuff 
...
## it's not giving the right number of observations (ughhhhhhhhhhhhh)
```
...
empty_data <- empty_data %>% 
  mutate(ID = row_number()) ## add a unique ID number to each state-year combo (an ID variable that will hopefully make joining easier
```

```{r}
## now joining things with the empty dataset

population_full <- population %>% 
  full_join(empty_data) ## everything is joined with the empty dataset (slay)

population_dems_full <- population_dems %>% 
  full_join(empty_data) ## this also joined correctly

income1_full <- income1 %>% 
  full_join(empty_data) %>% 
  select(-c(statefip, stateno, state_icpsr))
...
initiatives_full <- initiatives %>% 
  full_join(empty_data) %>% 
  select(-c(initiaitves, mean, sd))

unified_full <- unified %>% 
  full_join(empty_data)


evangelical <- evangelical2 %>% 
  filter(year %in% c(2014, 2015) & is.na(state_name)) %>% 
  full_join(crosswalk, by = "state") %>% 
  select(-c(state_fips.x, state_fips.y, state_name.x, state_icpsr.y, state_icpsr.x, stateno)) %>% 
  filter(!state %in% c("GU", "PR", "DC", "VI"))
...
evangelical2 <- evangelical2 %>% 
  ...
  select(-c(state_fips, state_icpsr))

evangelical2 <- evangelical2 %>% 
  full_join(evangelical)

evangelical_full <- evangelical2 %>% 
  full_join(empty_data)

covariates <- population_full %>% 
  full_join(population_dems_full) %>% 
  full_join(education_full) %>% 
  full_join(income1_full, by = c("state_name", "year", "ID")) %>% 
  full_join(evangelical_full) %>% 
  full_join(initiatives_full, by = c("state_name", "year", "ID")) %>% 
  full_join(pollib_full, by = c("state_name", "year", "ID")) %>% 
  full_join(unified_full, by = c("state_name", "year", "ID")) %>% 
  full_join(bowen_full, by = c("state_name", "year", "ID"))

covariates <- covariates %>% 
  select(-c(state.x, state_fips.x, state_icpsr.x, statefip.x, stateno.x, state.y, state.x.x, state.y.y, state.x.x.x, state_fips.y, stateno.y, state_icpsr.y, state_fips.y, state_fips.y.y, state_fips.y, state_icpsr.y.y, state_icpsr.x.x, state.y.y.y, state_fips.x.x))

## now gotta get the exisitng data in with the new data

export(covariates, "covariates.csv")

existing2 <- existing %>% 
  filter(year >= 1987) %>% 
  select(c(population_total, pollib_median, incomepcap, std_bowen1, std_bowen2, std_init_use, state, state_name, year, pct_black))


```

```{r}
## need to fix the initiatives data (why is it like this)

complete_covars <- import("./datasets/complete_covars.csv")

std_inits <- import("./datasets/initiatives2023.csv")  ## initiatives data

empty_data <- import("./datasets/EmptyData.csv") ## import the empty data

empty_data <- empty_data %>% 
  rename(state_name = "State", year = "Year") 
...
## export(empty_data, "empty_data.csv") export it so I don't have to mess with it again
```

```{r}
complete_covars <- complete_covars %>% 
  select(-c(state.y, pctblack.x, statefip.y, std_init_use, init, initiatives)) ## drop the extra variables and the std_init_use variable

std_inits <- std_inits %>% 
  full_join(empty_data) 

std_inits <- std_inits %>% 
  select(-c(mean, sd, initiaitves, state)) %>% 
  filter(year >= 1987)
...
complete_covars <- complete_covars %>% 
  left_join(std_inits, by = c("ID", "state_name", "year")) ## re-join with the complete covars 

##export(complete_covars, "complete_covars.csv") ## export it 
