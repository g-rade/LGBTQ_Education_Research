...
state_names <- import("./datasets/ces_data.dta") %>% 
  ...
  mutate(evangelical_pop = as.numeric(evangelical_pop*100), state = str_squish(state), state_name = str_squish(state_name)) ## name file for all 

var_label(state_names$evangelical_pop) <- NULL
attributes(state_names$year) <- NULL
state_names$state_name[state_names$state_name == ""] <- NA
...
names2014 <- state_names %>% 
  select(state_name) %>% 
  distinct() %>% 
  mutate(year = 2014) ## filling in the full state names for 2014
...
names2015 <- state_names %>% 
  ...
  distinct() %>% 
  mutate(year = 2015) %>% 
  filter(!is.na(state_name))## filling in the full state names for 2015

state_names <- state_names %>% 
  full_join(names2014, by = year) %>% 
  full_join(names2015) %>% 
  filter(!is.na(state_name))

race_health <- import("./datasets/race_health_data.dta") ## covariates
...
risk_set <- policy_risk_set %>% ## join the risk set to the name file
  left_join(just_names) %>% ## at this point there are no missing values in the state-name or state variables
