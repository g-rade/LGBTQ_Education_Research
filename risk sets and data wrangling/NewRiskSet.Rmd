...
population <- risk_states %>% 
  select(year, statename, population_total)
population <- population %>% 
  mutate(statename = str_squish(toupper(statename))) %>% 
  rename(state_name = "statename")

## for some reason when i join this with population2022 it doesn't work past 2020 ughhhhhhhh i hate correlates of state policy 
```
