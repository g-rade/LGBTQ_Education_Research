...
covars <- population_all %>%  
  ...
  full_join(bowen) %>% 
  full_join(education) %>% 
  left_join(crosswalk)## join everything together
...
pop_dems1 <- existing %>% 
  select(state_name, state, statefip, state_icpsr, stateno, year, pct_black, pctblack)
...
population_dems <- population_dems %>% 
  filter(!state %in% c("GU", "VI", "DC", "PR"))

population_dems <- population_dems %>% 
  distinct(state_name, state, year, .keep_all = TRUE)
